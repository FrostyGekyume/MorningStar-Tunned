...
moment.tz.setDefault('Brasil/São Paulo').locale('id')
...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const mess = {
            wait: '[ ESPERE ] O BOT Pode Acabar Sendo Um Pouco Lento Por Conta De Muito Uso!!',
            error: {
                St: '[❕] Envie A Imagem Com A Legenda !sticker Ou Então Marque Uma Imagem Para Virar Figurinha!',
                Qm: '[❕] ERRO!! Talvez O Tema Não Esteja Disponível?',
                Yt3: '[❕] ERRO!! Não Foi Possível Converter Para mp3!',
                Yt4: '[❕] Ocorreu Um Erro! Talvez O Erro Tenha Sido Causado Pelo Sistema!',
                Ig: '[❕] ERRO!! A Conta Talvez Esteja Privada!',
                Ki: '[❕] O BOT Precisa Ser Um Admin Do Grupo!',
                Ad: '[❕] Erro Ao aAdicionar Membro! Talvez A Conta Esteja Privada!',
                Iv: '[❕] Link Invalido!!'
            }
        }
        ...
        const ownerNumber = ["5519996503657@c.us","5519996503657"]
        ...
        case '!sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    const mediaData = await decryptMedia(message, uaOverride)
                    client.reply(from, '[ ESPERE ] Isso Pode Levar ± 1 Minuto!!', id)
                    ...
                    client.reply(from, '[❗] O Vídeo Só Pode Ter Ao Máximo 10 Segundos!!', id)
                )
            }
            break
	    case '!stickernobg':
	    ...
	    if (isMedia) {
            ...
            break
        case '!tts':
            if (args.length === 1) return client.reply(from, 'Coloque Junto Um Idioma! Exemplo: !tts pt Project MornigStar         (Idiomas Disponíveis Atualmente: id, en, jp e ar)')
	    ...
	    const ttsJp = require('node-gtts')('ja')
            ...
            if (dataText.length > 500) return client.reply(from, 'Texto Muito Grande!', id)
        ...
        case '!nulis':
            ...
            break
        case '!ytmp3':
            if (args.length === 1) return client.reply(from, 'Envie !ytmp3 Junto De Um Link Do YouTube!')
            ...
            try {
                ...
                if (resp.error) {
                    ...
                    if (Number(filesize.split(' MB')[0]) >= 30.00) return client.reply(from, 'Desculpe, a duração do vídeo excedeu o limite máximo!', id)
                    client.sendFileFromUrl(from, thumb, 'thumb.jpg', `➸ *Título* : ${title}\n➸ *Tamanho* : ${filesize}\n\nEspere! O Envio Pode Levar Alguns Minutos!.`, id)
            ...
            break
        case '!ytmp4':
            if (args.length === 1) return client.reply(from, 'Envie !ytmp4 Junto De Um Link Do YouTube!')
            ...
            try {
                ...
                if (ytv.error) {
                    ...
                    if (Number(ytv.filesize.split(' MB')[0]) > 40.00) return client.reply(from, 'Desculpe, a duração do vídeo excedeu o limite máximo!', id)
        ...
        case '!cuaca':
            ...
            break
        case '!fb':
            if (args.length === 1) return client.reply(from, 'Envie !fb Junto De Um Link Do Facebook', id)
        ...
        case '!creador':
            ...
            break
        case '!ig':
            if (args.length === 1) return client.reply(from, 'Envie !ig Junto De Um Link Do Instagram!')
        ...
        case '!nsfw':
            ...
            if (!isGroupAdmins) return client.reply(from, 'Este comando só pode ser usado pelo Admin!', id)
            if (args.length === 1) return client.reply(from, 'Selecione ativar ou desativar!', id)
            if (args[1].toLowerCase() === 'ativar') {
                nsfw_.push(chat.id)
                fs.writeFileSync('./lib/NSFW.json', JSON.stringify(nsfw_))
                client.reply(from, 'O Comando NSWF foi ativado com sucesso neste grupo! envie o comando !nsfwMenu para descobrir o menu', id)
            } else if (args[1].toLowerCase() === 'desativar') {
                nsfw_.splice(chat.id, 1)
                fs.writeFileSync('./lib/NSFW.json', JSON.stringify(nsfw_))
                client.reply(from, 'NSFW Desativado!!', id)
            } else {
                client.reply(from, 'Selecione ativar ou desativar!', id)
            }
            break
        case '!bv':
            if (!isGroupMsg) return client.reply(from, 'Este comando só pode ser usado em grupos!', id)
        ...
        case '!nsfwmenu':
            ...
            break
        case '!igstalk':
            if (args.length === 1)  return client.reply(from, 'Digite Um Username! *!igStalk @username*\nExemplo: *!igStalk @frostynid*', id)
        ...
        case '!infogempa':
            ...
            break
        case '!anime':
            if (args.length === 1) return client.reply(from, 'Diga O Anime Desejado! \nExemplo : *!anime darling in the franxx*', id)
            ...
            if (args.length === 2) {
                ...
                if (cek === true)  {
                    try {
                        ...
                        var teks = `*Título* : ${title}\n\n*Paródia* : ${parodies}\n\n*Tags* : ${tags.join(', ')}\n\n*Artista* : ${artists.join(', ')}\n\n*Grupo* : ${groups.join(', ')}\n\n*Idioma* : ${languages.join(', ')}\n\n*Categories* : ${categories}\n\n*Link* : ${link}`
                        ...
                        client.reply(from, '[❗] Algo deu errado, talvez o código esteja errado', id)
                    }
                } else {
                    client.reply(from, '[❗] Código errado!')
                }
            } else {
                client.reply(from, '[ ERRADO ] Você Deve Por O Codigo Junto! *!nh [código]*')
            }
        	break
        case '!brainly':
            if (args.length >= 2){
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		client.reply(from, 'Desculpe, não sei que anime é esse', id)
                	}
                ...
                client.sendFile(from, './media/img/tutod.jpg', 'Tutor.jpg', 'Exemplo!', id)
            }
            break
        case '!quotemaker':
            arg = body.trim().split('|')
            if (arg.length >= 4) {
                ...
                client.reply(from, 'Uso: \n!quotemaker |texto|marca d água|tema\n\nEx :\n!quotemaker |Frosty|bicit|random', id)
            }
            break
        case '!link':
            ...
            break
        case '!bc':
            if (!isOwner) return client.reply(from, 'Este comando é apenas para o dono!', id)
            let msg = body.slice(4)
            const chatz = await client.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await client.getChatById(ids)
                if (!cvk.isReadOnly) await client.sendText(ids, `[ TM Project MornigStar ]\n\n${msg}`)
            }
        ...
        case '@everyone':
            ...
            let hehe = '• Acorda Cambada De Macaco, O ADM Ta Chamando?\n'
            for (let i = 0; i < groupMem.length; i++) {
                hehe += '➥'
                hehe += ` @${groupMem[i].id.replace(/@c.us/g, '')}\n`
            }
            hehe += '〘 𝙿𝚛𝚘𝚓𝚎𝚌𝚝 𝙼𝚘𝚛𝚗𝚒𝚐𝚂𝚝𝚊𝚛 – 𝙱𝙾𝚃 〙'
            await client.sendTextWithMentions(from, hehe)
            break
        case '!banall':
            if (!isGroupMsg) return client.reply(from, 'Este comando só pode ser usado em grupos!', id)
            const isGroupOwner = sender.id === chat.groupMetadata.owner
            if (!isGroupOwner) return client.reply(from, 'Este comando só pode ser usado pelo dono do grupo!', id)
            ...
            for (let i = 0; i < allMem.length; i++) {
                if (groupAdmins.includes(allMem[i].id)) {
                    console.log('Ops Esse É ADM')
                } else {
                    await client.removeParticipant(groupId, allMem[i].id)
                }
            }
            client.reply(from, 'MorningStar Arquivamentos🤑😏🐊🥶🥶', id)
            break
        case '!leaveall':
            if (!isOwner) return client.reply(from, 'Este Comando Só Pode Ser Usado Pelo Dono Do BOT', id)
            ...
            client.reply(from, 'Sai De Todos Os Grupos!', id)
            break
        case '!clearall':
            if (!isOwner) return client.reply(from, 'Este Comando Só Pode Ser Usado Pelo Dono Do BOT', id)
        ...
        case '!add':
            ...
            if (args.length === 1) return client.reply(from, 'Para usar este recurso, envie o comando! Adicionar 5519xxxxxxxxx', id)
            if (!isGroupAdmins) return client.reply(from, 'Este comando só pode ser usado por administradores de grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este comando só pode ser usado quando o bot se torna administrador', id)
            try {
        ...
        case '!promote':
            ...
            if (mentionedJidList.length === 0) return client.reply(from, 'Para usar este recurso, envie o comando !promote @membro', id)
            ...
            await client.sendTextWithMentions(from, `Pedidos aceitos, adicionados @${mentionedJidList[0]} como admin.`)
            break
        case '!demote':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado por administradores de grupo', id)
        ...
        case '!join':
            ...
            if (args.length < 2) return client.reply(from, 'Use !join link \n\nEx:\n!join https://chat.whatsapp.com/blablablablablabla', id)
            const link = args[1]
            const key = args[2]
            ...
            if (check.size < minMem) return client.reply(from, 'Membros do grupo não superiores a 30, os bots não podem entrar', id)
            if (check.status === 200) {
                await client.joinGroupViaLink(link).then(() => client.reply(from, 'O bot vai entrar em breve!'))
            } else {
                client.reply(from, 'Link Invalido!', id)
            }
            break
        case '!delete':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado por administradores de grupo', id)
            if (!quotedMsg) return client.reply(from, 'Errado!! Envie o comando !delete [marcando mensagem do bot]', id)
            if (!quotedMsgObj.fromMe) return client.reply(from, 'Errado !!, Bots não podem deletar chats de outros usuários!', id)
        ...
        case '!getses':
            ...
            break
        case '!letra':
            if (args.length == 1) return client.reply(from, 'Use !letra nome da música', id)
        ...
        case '!loli':
            ...
            client.sendFileFromUrl(from, loli.result, 'loli.jpeg', 'O-Onii-Chan?', id)
            break
        case '!waifu':
        ...
        case '!randomhentai':
            if (isGroupMsg) {
                if (!isNsfw) return client.reply(from, 'Comando NSFW não foi ativado neste grupo!', id)
        ...
        case '!url2img':
            ...
            if (args.length === 1) return client.reply(from, 'Use !url2img https://google.com', id)
        ...
        case '!quotesnime':
            ...
            skya_ = skya.data
            client.reply(from, `➸ *Frase* : ${skya_.quote}\n➸ *Personagem* : ${skya_.character}\n➸ *Anime* : ${skya_.anime}`, id)
            break
        case '!meme':
